...
3) `required(...)` or `optional(...)` converts the constructor to a `CommandArgument` and registers it.
    * `required(...)` simply runs the child arc on the argument builder, then attaches it to the parent with `.then`
    * `optional(...)` does the same, but also runs the arc on the parent directly with a `null` argument
4) The argument passed to the child is `ArgumentAccessor` -- a arc that can be thought of as a
...
defer creating it
and allowing it to be passed to arcs instead of `ArgumentAccessor`. The command context would then be passed in as a
